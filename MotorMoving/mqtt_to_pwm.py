...
# ---------------- Configuration ----------------
# BCM pin numbers
ENABLE_PINS: List[int] = [19, 26, 20, 21]  # 4 enable pins for 4 motors (avoiding 18,22,27 used by gimbal)
...
class GimbalController:
    ...
    def handle_command(self, cmd: dict, client: mqtt.Client):
        ...
        degrees = float(cmd.get("degrees", 10))
...
def handle_command(ctrl: MotorController, gimbal_ctrl: GimbalController, cmd: dict, client: mqtt.Client):
    t = cmd.get("type", "all")
    action = cmd.get("action")
    
    # Handle gimbal commands
    if t == "gimbal":
        gimbal_ctrl.handle_command(cmd, client)
        return
...
def main():
    ...
    def on_message(cli, _userdata, msg):
        ...
        if not cmd:
            ...
            return
        try:
            handle_command(ctrl, gimbal_ctrl, cmd, cli)
        except Exception as e:
